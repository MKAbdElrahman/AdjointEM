...
\begin{equation}\label{part_intg}
 \int_{0}^{T}  \lambda^T C \frac{\partial}{\partial \epsilon}   \frac{d \mathbf{X}}{dt} dt = \lambda^T C \frac{\partial \mathbf{X}}{\partial \epsilon}|^T_0 - \int_{0}^{T}
...
If we also imposed the terminal condition $\lambda^T(T) C = 0$, the objective function gradient is 
 ...
 \frac{d G}{d\epsilon} = \int_{0}^{T} \left[  \frac{\partial g}{\partial \epsilon} + \lambda^T \frac{d C}{d \epsilon} \frac{d \mathbf{X}}{dt}  \right]  + \ \kappa^T \frac{\partial I}{\partial \epsilon}
 \end{equation}
\end{document}